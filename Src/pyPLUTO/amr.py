		...
		for i in range(il+1)[::-1]:
			...
			if (i == il):
				...
				try:
					...
					if (dim == 3):zstr = fl.attrs.get('g_x3stretch')
				except:
				...
				if ((ib > iend) or (ie < ibeg) or \
					...
					for iv in range(nvar):
						vars[ib0-ibeg:ie0-ibeg+1,jb0-jbeg:je0-jbeg+1,kb0-kbeg:ke0-kbeg+1,iv] = \
							self._congrid(q1[:,:,:,iv].squeeze(),new_shape,method='linear',minusone=True).reshape((ie0-ib0+1,je0-jb0+1,ke0-kb0+1))
					ncount = ncount+szb
