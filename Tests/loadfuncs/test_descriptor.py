"""Test of the descriptor.py file."""
...
def test_load_outfile_oneoutput(data_dir: Path):

    Data = pp.Load(path=data_dir / "multiple_outputs", text=False)
...
def test_load_outfile_moreoutputs(data_dir: Path):
    Data = pp.Load(
        [0, "last"],
        path=data_dir / "multiple_outputs",
        text=False,
        endian="little",
    )
...
def test_load_outfilevtk_oneoutput(data_dir: Path):
    Data = pp.Load(
        path=data_dir / "multiple_outputs", text=False, datatype="vtk"
    )
...
def test_load_outfiletab_oneoutput(data_dir: Path):
    Data = pp.Load(
        path=data_dir / "multiple_outputs", text=False, datatype="tab"
    )
