...
def test_2D_singledbl(data_dir: Path):
    Data = pp.Load(0, path=data_dir / "single_file", text=False)
    for num, var in enumerate(varslist):
        npt.assert_array_equal(getattr(Data, var), exact_vars[var])


# Test multiple files, dbl output
def test_2D_multipledbl(data_dir: Path):
    Data = pp.Load(0, path=data_dir / "multiple_files", text=False)
    for num, var in enumerate(varslist):
        npt.assert_array_equal(getattr(Data, var), exact_vars[var])


# Test single file, vtk output
def test_singlevtk(data_dir: Path):
    Data = pp.Load(0, path=data_dir / "single_file", datatype="vtk", text=False)
    for num, var in enumerate(varslist):
        npt.assert_allclose(getattr(Data, var), exact_vars[var])


# Test multiple files, vtk output
def test_multiplevtk(data_dir: Path):
    Data = pp.Load(
        0, path=data_dir / "multiple_files", datatype="vtk", text=False
    )
    for num, var in enumerate(varslist):
        npt.assert_allclose(getattr(Data, var), exact_vars[var])
...
def test_dblh5(data_dir: Path):
    Data = pp.Load(
        0, path=data_dir / "single_file", datatype="dbl.h5", text=False
    )
    for num, var in enumerate(varslist):
        npt.assert_array_equal(getattr(Data, var), exact_vars[var])
...
# Test flt.h5 output
def test_flth5(data_dir: Path):
    Data = pp.Load(
        0, path=data_dir / "single_file", datatype="flt.h5", text=False
    )
    for num, var in enumerate(varslist):
        npt.assert_allclose(getattr(Data, var), exact_vars[var])
...
def test_tab(data_dir: Path):
    Data = pp.Load(0, path=data_dir / "single_file", datatype="tab", text=False)
    for num, var in enumerate(varslist):
        npt.assert_allclose(getattr(Data, var), exact_vars[var])
...
def test_dblh5alone(data_dir: Path):
...
def test_flth5alone(data_dir: Path):
    with pytest.warns(UserWarning, match=warn):
        Data = pp.Load(
            path=data_dir / "single_file",
            text=False,
...
def test_tabalone(data_dir: Path):
    vars_conversion = {
        "rho": "var1",
        "vx1": "var2",
        "vx2": "var3",
...
def test_tab_crlf_line_endings(tmp_path, data_dir: Path):
    src = data_dir / "single_file"
...
def test_tab1D(data_dir: Path):
    Data = pp.Load(
        0, path=data_dir / "multiple_outputs", datatype="tab", text=False
    )
...
def test_multipleout(data_dir: Path):
    Data = pp.Load(
        [0, 1], path=data_dir / "multiple_outputs", datatype="dbl", text=False
    )
