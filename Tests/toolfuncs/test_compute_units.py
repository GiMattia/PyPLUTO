...
def test_units_default_behavior(data_dir: Path):
    D = _load_single(data_dir, units=False)
    assert isinstance(D.units, dict)
    assert not hasattr(D.rho, "unit")


def test_units_true_attach_all_known(data_dir: Path):
    u = pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=True)
    assert hasattr(D.rho, "unit")
    assert D.rho.unit.is_equivalent(u.g / u.cm**3)


def test_units_single_var(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units="rho")
    assert hasattr(D.rho, "unit")
    assert not hasattr(D.prs, "unit")


def test_units_list_var(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=["rho", "prs"])
    assert hasattr(D.rho, "unit")
    assert hasattr(D.prs, "unit")
    assert not hasattr(D.vx1, "unit")


def test_skip_units(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=True, skip_units="rho")
    assert not hasattr(D.rho, "unit")
    assert hasattr(D.prs, "unit")


def test_to_astropy_units(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=False)
...
def test_double_conversion_protection(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=False)
    D.to_astropy_units("rho")
    first = D.rho.copy()
...
def test_to_code_units_roundtrip(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=False)
    rho0 = np.array(D.rho)
    D.to_astropy_units("rho")
...
def test_unknown_unit_explicit_raises(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(data_dir, units=False)
...
def test_units_userdef_from_load_kwargs(data_dir: Path):
    pytest.importorskip("astropy.units")
    D = _load_single(
        data_dir,
        units=False,
