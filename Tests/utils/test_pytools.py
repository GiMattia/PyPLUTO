...
def test_ring_posix(monkeypatch):
    """Covers the Linux/macOS branch, which shells out to 'play'."""
    called = {}
    monkeypatch.setattr(pt, "windows", None)
    monkeypatch.setattr(pt.os, "name", "posix")
    monkeypatch.setattr(
        pt.os, "system", lambda cmd: called.setdefault("cmd", cmd)
    )

    pt.ring(length=0.2, freq=220)

    assert "play" in called["cmd"]
    assert "0.2" in called["cmd"]
    assert "220" in called["cmd"]


def test_ring_unsupported_os(monkeypatch):
    """Covers the branch for an OS that has neither winsound nor 'play'."""
    monkeypatch.setattr(pt, "windows", None)
    monkeypatch.setattr(pt.os, "name", "java")
    with pytest.warns(UserWarning, match="not implemented for this OS"):
        pt.ring()
